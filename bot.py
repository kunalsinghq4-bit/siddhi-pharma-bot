...
def get_due_customers():
    customers = supabase_get("reorder_customers", "paused=eq.false&sent_today=eq.false")
    due_today = []
    for c in customers:
...
def mark_sent(customer_id):
    ...
    supabase_patch("reorder_customers", customer_id, {"sent_today": True, "last_sent_date": today})
